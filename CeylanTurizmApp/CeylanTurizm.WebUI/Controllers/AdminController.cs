...
namespace CeylanTurizm.WebUI.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public AdminController(IBusService busService, ICityService cityService, IExpeditionService expeditionService, ITicketSalesService ticketSalesService)
        {
        ...
        public IActionResult TicketSalesIndex()
        {
            return View();
        }
        public IActionResult ExpeditionListByTicket(Expedition model)
        {
            if (!String.IsNullOrEmpty(model.ExpeditionDate))
            {
                return View(_expeditionService.SearchByDate(model.ExpeditionDate));
            }
            if (model.ExpeditionId != null)
            {
                return View(_expeditionService.SearchById(model.ExpeditionId));
            }
            if (model.BusId != null)
            {
                return View(_expeditionService.SearchByBusId(model.BusId));
            }
            ViewBag.Result = "Lütfen En Az Bir Kriter Seçiniz";
            return RedirectToAction("TicketSalesIndex");
        }
        public IActionResult TicketsInformation(int id)
        {
            return View(_ticketSalesService.TicketsInformation(id));
        }
        public IActionResult TicketEdit(int id)
        {
            return View(_ticketSalesService.PurchasedTickets(id));
        }
        [HttpPost]
        public IActionResult TicketEdit(TicketSales ticket)
        {
            TicketSales ticketSales = new TicketSales()
            {
                ...
                customerTelNo = ticket.customerTelNo,
                customerTcNo = ticket.customerTcNo,
            };
            _ticketSalesService.Update(ticketSales);
            return Redirect($"/Admin/TicketsInformation/{ticket.ExpeditionId}");
        }
        public IActionResult TicketDelete(int id)
        {
            _ticketSalesService.Delete(_ticketSalesService.GetById(id));
            return RedirectToAction("TicketSalesIndex");
        }
    }
}
