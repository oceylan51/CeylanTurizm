...
namespace CeylanTurizm.Data.Concrete.EfCore
{
    public class EfCoreExpeditonRepository : EfCoreGenericRepository<Expedition, CeylanTurizmDbContext>, IExpeditionRepository
    {
        ...
        public List<Expedition> SearchByBusId(int id)
        {
            using (var context = new CeylanTurizmDbContext())
            {
                return context.Expeditions.Where(x => x.BusId == id).ToList();
            }
        }

        public List<Expedition> SearchByDate(string date)
        {
            using (var context = new CeylanTurizmDbContext())
            {
                return context.Expeditions.Where(x => x.ExpeditionDate == date).ToList();
            }
        }

        public List<Expedition> SearchById(int id)
        {
            using (var context = new CeylanTurizmDbContext())
            {
                return context.Expeditions.Where(x => x.ExpeditionId == id).ToList();
            }
        }
    }
}
